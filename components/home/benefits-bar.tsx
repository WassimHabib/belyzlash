...
export function BenefitsBar() {
  ...
  return (
    <section ref={ref} className="bg-brand-cream border-y border-brand-beige">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {benefits.map((benefit, i) => (
            ...
            >
              <div className="text-brand-gold">{benefit.icon}</div>
              <div>
                <p className="font-medium text-brand-black text-sm tracking-wide">{benefit.title}</p>
                <p className="text-brand-black/50 text-xs mt-1">{benefit.description}</p>
              </div>
            </div>
          ))}
        </div>
